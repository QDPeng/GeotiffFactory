...
jobject NativeDecoder::getBitmap() {
    ...
    if (decodeArea) {
        ...
        jclass decodeAreaClass = env->FindClass(
                "org/beyka/tiffbitmapfactory/TiffBitmapFactory$DecodeArea");
...
jstring NativeDecoder::charsToJString(char *chars) {
    ...
    jbyteArray array = env->NewByteArray((jsize) str.size());
    env->SetByteArrayRegion(array, 0, (jsize) str.size(), (const jbyte *) str.c_str());
